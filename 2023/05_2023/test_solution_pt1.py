...
def test_parse_map():
    ...
    assert parse_map(line) == Mapping(
        destination_start=50, source_start=98, range_length=2
    )
...
def test_parse_text():
    ...
    assert almanac.seed_to_soil == [
        Mapping(destination_start=52, source_start=50, range_length=48),
        Mapping(destination_start=50, source_start=98, range_length=2),
    ]
    assert almanac.temperature_to_humidity == [
        Mapping(destination_start=1, source_start=0, range_length=69),
        Mapping(destination_start=0, source_start=69, range_length=1),
    ]
